...
export const sweeperGame = {
  ...
  hint: 'ARROWS — 탐색 · SPACE / TAP — 조사',
  accessibility: '10열 6행의 심해 탐사 구역입니다. 방향키로 조사 위치를 옮기고 스페이스 또는 Enter로 칸을 조사합니다. 숫자는 주변 여덟 칸의 위험 개수이며, 데이터 로그 세 개를 찾은 뒤 오른쪽 아래 탈출 지점을 조사하면 성공합니다.',
  ...
  create({ context, width, height, onScore, onEnd, sound }) {
    ...
    function revealSafeRegion(startIndex) {
      ...
      while (pending.length) {
        ...
        if (!cell || cell.revealed || cell.kind !== 'safe') continue;
        cell.revealed = true;
        state.score += 35;
    ...
    function renderCell(cell, index) {
      ...
      let symbol = '·';
      if (cell.revealed && cell.kind === 'hazard') {
        symbol = '✕';
